...
async function handleWebhookService(body) {
    ...
    const { data: paymentRecord, error: updateError } = await supabaseAdmin_1.supabase
        .from("payments")
        .update({
        status,
    ...
    if (status === "approved" && paymentRecord) {
        ...
        if (originalPayload && paymentRecord.user_id) {
            try {
                const tickets = await (0, ticketService_1.generateTickets)({
                ...
                const { error: ticketUpdateError } = await supabaseAdmin_1.supabase
                    .from("payments")
